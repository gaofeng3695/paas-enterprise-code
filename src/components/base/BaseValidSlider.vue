...
<script>
  ...
  export default {
    ...
    computed: {
      tip () {
        return this.success ? '验证通过' : '请按住滑块，拖动到最右端';
      },
      className () {
        return this.success ? 'moveSuccess' : 'moveBefore';
      },
      moveStyle () {
        return {
    ...
    methods: {
      ...
      doMove (e) {
        if (!this.moving || this.success) return;
        this.moveWidth = e.clientX - this.startX; // 滑块的滑动距离
        // 临界值小于
        if (this.moveWidth <= 0) {
          this.moveWidth = 0;
        }
        let moveboxW = document.getElementsByClassName('movebox')[0].clientWidth;
        let moveW = document.getElementsByClassName('move')[0].clientWidth;
        // var width = $('.movebox').width() - $('.move').width()
        var width = moveboxW - moveW;
        // 临界值大于
